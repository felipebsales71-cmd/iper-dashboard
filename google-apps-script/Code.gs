...
const IPER_WEBHOOK_URL =
	"https://iper-dashboard.pages.dev/api/dashboard-refresh";
const IPER_MONITORED_SHEETS = ["Banco de Dados", "Sistema Golaço"];
...
function doGet() {
	try {
		...
		const sistema = spreadsheet.getSheetByName("Sistema Golaço");

		if (!banco || !sistema) {
			throw new Error(
				'As abas "Banco de Dados" e "Sistema Golaço" são obrigatórias.',
			);
		}
...
function gerarSegredoWebhook() {
	...
	if (!spreadsheet) {
		throw new Error(
			"Abra este script a partir da própria Google Planilha.",
		);
	}
...
function notificarDashboard_(eventPayload) {
	const properties = PropertiesService.getScriptProperties();
	const webhookUrl =
		properties.getProperty("DASHBOARD_WEBHOOK_URL") || IPER_WEBHOOK_URL;
	const webhookSecret = properties.getProperty("DASHBOARD_WEBHOOK_SECRET");

	if (!webhookSecret) {
		throw new Error(
			"Execute gerarSegredoWebhook() antes de instalar a integração.",
		);
	}
	...
	if (status < 200 || status >= 300) {
		throw new Error(
			"Cloudflare respondeu HTTP " + status + ": " + body.slice(0, 500),
		);
	}
...
function getSpreadsheet_() {
	...
	if (!active) {
		throw new Error(
			"ID da planilha não configurado. Execute gerarSegredoWebhook().",
		);
	}
	return active;
}
...
function readBancoDeDados_(sheet) {
	...
	return values.slice(1).reduce(function (records, row) {
		...
		const adjustment =
			number_(value_(row, col, "ENCARGO P")) -
			number_(value_(row, col, "DEDUCAO P")) +
			number_(value_(row, col, "ENCARGO S")) -
			number_(value_(row, col, "DEDUCAO S"));
		...
		records.push({
			...
			calculationBase: round2_(
				number_(value_(row, col, "BASE DE CALCULO")),
			),
			...
			ingress: title_(
				text_(value_(row, col, "INGRESSO")) || "Não informado",
			),
		});
		return records;
	}, []);
