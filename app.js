...
function dashboardAgency(row) {
	...
	if (
		entity === "GOVERNO" ||
		normalizedAgency === "GOVERNO" ||
		normalizedAgency === "GOV"
	)
		return "GOV";
	...
	if (
		!entity &&
		normalizedAgency.startsWith("SE") &&
		normalizedAgency !== "SESAU"
	)
		return "GOV";
...
function compact(value) {
	...
	if (abs >= 1_000_000_000)
		return `${(number / 1_000_000_000).toLocaleString("pt-BR", { maximumFractionDigits: 1 })} bi`;
	if (abs >= 1_000_000)
		return `${(number / 1_000_000).toLocaleString("pt-BR", { maximumFractionDigits: 1 })} mi`;
	if (abs >= 1_000)
		return `${(number / 1_000).toLocaleString("pt-BR", { maximumFractionDigits: 1 })} mil`;
...
async function loadData({ preserveFilters = false, announce = false } = {}) {
	...
	try {
		...
		live =
			payload?.meta?.connection === "live" ||
			payload?.meta?.updateMode === "webhook-r2";
	} catch (error) {
		...
		if (!response.ok)
			response = await fetch("data/demo.json", { cache: "no-store" });
		payload = await response.json();
...
function renderIframeKpis() {
	const years = uniqueValues(state.allRecords, "year").sort(
	...
	const months = uniqueValues(yearRows, "month").sort(
	...
	$("#competenceLabel").textContent = selectedMonth
		? selectedMonth.replace("/", " de ")
		: "—";

	$("#serversPeriodLabel").textContent = selectedMonth
		? selectedMonth.replace("/", " de ")
		: "—";
}
...
function populateSelect(select, key, defaultLabel) {
	...
	uniqueValues(state.allRecords, key).forEach((value) =>
...
function populateFilters() {
	...
	year.innerHTML = "";
	uniqueValues(state.allRecords, "year")
		.sort((a, b) => Number(b) - Number(a))
		.forEach((value) => year.add(new Option(value, value)));
	if (
		currentYear &&
		[...year.options].some((option) => option.value === currentYear)
	)
	...
	FUND_ORDER.filter((name) =>
		state.allRecords.some((row) => row.fund === name),
	).forEach((value) => fund.add(new Option(value, value)));
	uniqueValues(state.allRecords, "fund")
		.filter((value) => !FUND_ORDER.includes(value))
		.forEach((value) => fund.add(new Option(value, value)));
	if ([...fund.options].some((option) => option.value === currentFund))
		fund.value = currentFund;
	...
	if ([...serverType.options].some((option) => option.value === currentType))
		serverType.value = currentType;
}
function setDefaultPeriod() {
	if (!$("#yearFilter").value) {
		const latestYear =
			uniqueValues(state.allRecords, "year").sort(
				(a, b) => Number(b) - Number(a),
			)[0] || "";
...
function syncMonthOptions(preserve = false) {
	...
	const months = uniqueValues(
		state.allRecords.filter((row) => !year || row.year === year),
		"month",
	).sort((a, b) => monthKey(a) - monthKey(b));
	select.innerHTML = "";
	months.forEach((value) =>
		select.add(new Option(value.split("/")[0], value)),
	);
...
function getScopes() {
	...
	const filtered = state.allRecords.filter((row) => {
		...
		if (filters.entityFilter && row.entity !== filters.entityFilter)
			return false;
		if (filters.agencyFilter && row.agency !== filters.agencyFilter)
			return false;
		if (filters.categoryFilter && row.category !== filters.categoryFilter)
			return false;
		if (filters.payrollFilter && row.payroll !== filters.payrollFilter)
			return false;
		if (
			filters.serverTypeFilter &&
			row.serverType !== filters.serverTypeFilter
		)
			return false;
	...
	const annualRows = filtered.filter(
		(row) => !selectedMonth || row.monthKey <= selectedMonthKey,
	);
	const competenceRows = filtered.filter(
		(row) => !selectedMonth || row.month === selectedMonth,
	);
	const result = {
		filters,
		filtered,
...
function renderKpis(scopes) {
	...
	const previousRows = scopes.filtered.filter(
		(row) => row.monthKey === previousMonthKey,
	);
	const previousRevenue = sum(previousRows, "revenue");
	const delta = previousRevenue
	...
	const monthName = scopes.selectedMonth
		...
		: "—";
	$("#annualPeriodLabel").textContent = scopes.selectedMonth
		? `Jan a ${scopes.selectedMonth}`
		: scopes.selectedYear;
...
function renderFundCompetence(scopes) {
	...
	const funds = (
		scopes.filters.fundFilter ? [scopes.filters.fundFilter] : FUND_ORDER
	).filter((fund) => state.allRecords.some((row) => row.fund === fund));
	const data = funds.map((fund) => ({
		fund,
		value: sum(
			scopes.competenceRows.filter((row) => row.fund === fund),
			"revenue",
		),
	}));
	if (!data.length || data.every((item) => item.value === 0))
		return renderEmpty(
			element,
			"Não há arrecadação para a competência selecionada.",
		);

	const w = 760,
		h = 250,
		p = { l: 64, r: 22, t: 28, b: 58 };
	const max = Math.max(...data.map((item) => item.value), 1);
	const slot = (w - p.l - p.r) / data.length;
	const barWidth = Math.min(100, slot * 0.46);
	...
	$$("[data-fund]", element).forEach((node) => {
		node.addEventListener("click", () => {
			$("#fundFilter").value = node.dataset.fund;
			renderAll();
		});
...
function renderFundAccumulated(scopes) {
	...
	const months = uniqueValues(scopes.filtered, "month")
		.filter((month) => monthKey(month) <= scopes.selectedMonthKey)
		.sort((a, b) => monthKey(a) - monthKey(b));
	const funds = (
		scopes.filters.fundFilter ? [scopes.filters.fundFilter] : FUND_ORDER
	).filter((fund) => state.allRecords.some((row) => row.fund === fund));
	if (!months.length || !funds.length)
		return renderEmpty(
			chart,
			"Não há dados acumulados para o período selecionado.",
		);

	legend.innerHTML = funds
		.map(
			(fund) =>
				`<button class="legend-button ${state.hiddenFunds.has(fund) ? "is-hidden" : ""}" type="button" data-toggle-fund="${escapeHtml(fund)}"><i style="background:${FUND_COLORS[fund] || "#58778c"}"></i>${escapeHtml(fund.replace("Fundo ", ""))}</button>`,
		)
		.join("");
	$$("[data-toggle-fund]", legend).forEach((button) =>
		button.addEventListener("click", () => {
			const fund = button.dataset.toggleFund;
			if (state.hiddenFunds.has(fund)) state.hiddenFunds.delete(fund);
			else state.hiddenFunds.add(fund);
		...
		.map((fund) => {
			...
			return {
				...
				values: months.map((month) => {
					running += sum(
						scopes.filtered.filter(
							(row) => row.fund === fund && row.month === month,
						),
						"revenue",
					);
	...
	if (!series.length)
		return renderEmpty(
			chart,
			"Todos os fundos estão ocultos. Use “Exibir todos”.",
		);

	const w = 760,
		h = 300,
		p = { l: 70, r: 24, t: 24, b: 56 };
	const max = Math.max(
		...series.flatMap((item) => item.values.map((point) => point.value)),
		1,
	);
	const x = (index) =>
		p.l + index * ((w - p.l - p.r) / Math.max(months.length - 1, 1));
	const y = (value) => h - p.b - (value / max) * (h - p.t - p.b);
	const grid = [0, 0.25, 0.5, 0.75, 1]
		.map((t) => {
			const yy = y(max * t);
	...
	const xLabels = months
		.map(
			(month, index) =>
				`<text class="axis-label" x="${x(index)}" y="${h - 22}" text-anchor="middle">${escapeHtml(month.split("/")[0])}</text>`,
		)
		.join("");
	const lines = series
		.map((item) => {
			...
			const circles = item.values
				.map(
					(point, index) =>
						`<circle tabindex="0" role="button" aria-label="${escapeHtml(item.fund)} em ${escapeHtml(point.month)}" class="fund-point" data-month="${escapeHtml(point.month)}" data-tip="${escapeHtml(item.fund)} · ${escapeHtml(point.month)} · ${escapeHtml(MONEY.format(point.value))}" cx="${x(index)}" cy="${y(point.value)}" r="5" fill="${color}"/>`,
				)
				.join("");
	...
	$$("[data-month]", chart).forEach((node) => {
		node.addEventListener("click", () => {
			$("#monthFilter").value = node.dataset.month;
			renderAll();
		});
		node.addEventListener("keydown", (event) => {
			if (event.key === "Enter" || event.key === " ") {
...
function renderServers(scopes) {
	...
	const matrix = funds.map((fund) => {
		...
		const values = SERVER_TYPES.map((type) => ({
			type,
			value: sum(
				rows.filter((row) => row.serverType === type),
				"servers",
			),
		}));
		return {
			fund,
			values,
			total: values.reduce((total, item) => total + item.value, 0),
		};
	});
	const max = Math.max(
		...matrix.flatMap((group) => group.values.map((item) => item.value)),
		1,
	);
	if (!matrix.length || matrix.every((group) => group.total === 0)) {
		container.innerHTML =
			'<div class="chart-empty">Não há quantitativo de servidores para a seleção.</div>';
		summary.innerHTML = "";
		return;
	}
	container.innerHTML = matrix
		.map(
			(group) =>
				`<section class="server-fund-group ${fundClass(group.fund)}"><div class="server-fund-title"><strong>${escapeHtml(group.fund)}</strong><span>${INTEGER.format(group.total)}</span></div>${group.values
					.map((item) => {
						const ratio =
							state.serverMode === "percent"
								? group.total
									? item.value / group.total
									: 0
								: item.value / max;
						const display =
							state.serverMode === "percent"
		...
		.join("");
	$$("[data-server-type]", container).forEach((button) =>
		button.addEventListener("click", () => {
	...
	const typeTotals = SERVER_TYPES.map((type) => ({
		type,
		value: sum(
			scopes.competenceRows.filter((row) => row.serverType === type),
			"servers",
		),
	}));
	summary.innerHTML = typeTotals
		.map((item) => {
			const klass =
				item.type === "Ativo"
					? "active"
...
function renderAgencyRanking(scopes) {
	...
	const rows =
		state.agencyScope === "competence"
			? scopes.competenceRows
			: scopes.annualRows;
	const search = state.agencySearch.trim().toLocaleLowerCase("pt-BR");
	let data = aggregate(rows, "agency", "revenue").sort(
		(a, b) => b.value - a.value,
	);
	if (search)
		data = data.filter((item) =>
			String(item.label).toLocaleLowerCase("pt-BR").includes(search),
		);
	const total = data.reduce((value, item) => value + item.value, 0);
	const max = data[0]?.value || 1;
	if (state.agencyLimit !== "all")
		data = data.slice(0, Number(state.agencyLimit) || 10);
...
function renderActiveChips(filters) {
	...
	const definitions = [
		["fundFilter", "Fundo", filters.fundFilter],
		["entityFilter", "Poder", filters.entityFilter],
		["agencyFilter", "Órgão", filters.agencyFilter],
		["categoryFilter", "Categoria", filters.categoryFilter],
		["payrollFilter", "Folha", filters.payrollFilter],
		["serverTypeFilter", "Tipo", filters.serverTypeFilter],
		["searchInput", "Pesquisa", filters.searchInput],
		["startDate", "De", filters.startDate],
		["endDate", "Até", filters.endDate],
	];
	const active = definitions.filter(([, , value]) => value);
	container.innerHTML = active
		.map(
			([id, label, value]) =>
				`<span class="filter-chip">${escapeHtml(label)}: ${escapeHtml(value)}<button type="button" data-clear-filter="${id}" aria-label="Remover filtro ${escapeHtml(label)}">×</button></span>`,
		)
		.join("");
...
function bindTooltips(root) {
	$$("[data-tip]", root).forEach((node) => {
		node.addEventListener("pointermove", (event) =>
			showTooltip(event, node.dataset.tip),
		);
...
function updateStoryMetrics() {
	...
	const months = uniqueValues(
		state.allRecords.filter((row) => row.year === latestYear),
		"month",
	).sort((a, b) => monthKey(a) - monthKey(b));
	const latestMonth = months.at(-1) || "";
	const annualRows = state.allRecords.filter(
		(row) =>
			row.year === latestYear && row.monthKey <= monthKey(latestMonth),
	);
	const competenceRows = state.allRecords.filter(
		(row) => row.month === latestMonth,
	);
	$$('[data-story-kpi="annual"]').forEach(
		(node) => (node.textContent = compact(sum(annualRows, "contribution"))),
	);
	$$('[data-story-kpi="servers"]').forEach(
		(node) =>
			(node.textContent = INTEGER.format(sum(competenceRows, "servers"))),
	);
	$$("[data-story-period]").forEach(
		(node) =>
			(node.textContent = latestMonth
				? `Até ${latestMonth}`
...
function resetFilters() {
	...
	$$("[data-agency-scope]").forEach((button) =>
		button.classList.toggle(
			"is-active",
			button.dataset.agencyScope === "annual",
		),
	);
	renderAll();
}
...
function initHeroStory() {
	...
	const sceneOpacity = (exact, target, spread = 0.72) =>
		clamp(1 - Math.abs(exact - target) / spread);
	...
	const update = () => {
		...
		const current = Math.min(panels.length - 1, Math.floor(exact + 0.5));
		state.heroScene = current;

		panels.forEach((panel, index) => {
			const opacity = index === 0 ? 0 : sceneOpacity(exact, index, 0.78);
		...
		composition.style.transform = `translateY(${latePhase * -7}vh) scale(${1 - latePhase * 0.075})`;
		composition.style.opacity = String(1 - latePhase * 0.28);

		logo.style.transform = `translate(-50%,-50%) translateY(${introPhase * -12}vh) scale(${1 - introPhase * 0.27 - latePhase * 0.08}) rotate(${exact * 0.35}deg)`;
		logo.style.opacity = String(1 - latePhase * 0.34);

		revenueCard.style.transform = `translateY(${(1 - agencyFocus) * 28}px) scale(${0.94 + agencyFocus * 0.06})`;
		revenueCard.style.opacity = String(agencyFocus * (1 - latePhase * 0.2));

		fundCard.style.transform = `translateY(${fundFocus * -18}px) scale(${0.96 + fundFocus * 0.06})`;
		fundCard.style.opacity = String(agencyFocus * (1 - latePhase * 0.2));

		agencyCard.style.transform = `translateY(${(1 - agencyFocus) * 28}px) scale(${0.94 + agencyFocus * 0.06})`;
		agencyCard.style.opacity = String(agencyFocus * (1 - latePhase * 0.2));

		bars.forEach((bar, index) => {
			const rise = clamp((exact - 0.35 - index * 0.055) / 0.75);
			bar.style.transform = `scaleY(${0.2 + rise * 0.8})`;
			bar.style.opacity = String(0.45 + rise * 0.55);
		});

		$(".ambient-grid").style.transform =
			`translate3d(${exact * -8}px,${exact * -5}px,0) scale(${1 + exact * 0.012})`;
		$(".scroll-hint").style.opacity = String(clamp(1 - progressValue * 5));
		$("#siteHeader").classList.toggle(
			"is-solid",
			progressValue > 0.86 || scrollY > innerHeight * 0.35,
		);
	};
...
function initEvents() {
	...
	$("#resetFundVisibility").addEventListener("click", () => {
		state.hiddenFunds.clear();
		renderFundAccumulated(getScopes());
	});
	$$("[data-server-mode]").forEach((button) =>
		button.addEventListener("click", () => {
			state.serverMode = button.dataset.serverMode;
			$$("[data-server-mode]").forEach((item) =>
	...
	$$("[data-agency-scope]").forEach((button) =>
		button.addEventListener("click", () => {
			...
			$$("[data-agency-scope]").forEach((item) =>
				item.classList.toggle("is-active", item === button),
			);
			renderAgencyRanking(getScopes());
		}),
	);
	$("#agencyLimit").addEventListener("change", (event) => {
		state.agencyLimit = event.target.value;
		renderAgencyRanking(getScopes());
	});
	$("#agencyRankingSearch").addEventListener("input", (event) => {
	...
	if (dialog) {
		$$("[data-open-admin]").forEach((button) =>
