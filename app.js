...
async function loadData() {
  ...
  try {
    ...
    live =
      payload?.meta?.connection === "live" || payload?.meta?.isDemo === false;
  } catch (error) {
    console.warn(
      "Fonte ao vivo indisponível; usando contingência local.",
      error,
    );
  ...
  applyFilters();
  updateStoryKpis();
  const parsedDate = state.meta.updatedAt
    ? new Date(state.meta.updatedAt)
    : null;
  const when =
...
function monthKey(label = "") {
  ...
  return Number(y || 0) * 12 + monthOrder.indexOf(m);
}
...
function renderKpis() {
  ...
  const entities = new Set(state.filtered.map((r) => r.agency).filter(Boolean))
    .size;
  const monthly = aggregate(state.filtered, "month", "revenue").sort(
    (a, b) => monthKey(a.label) - monthKey(b.label),
  );
  ...
  $("#revenueDelta").textContent =
    monthly.length > 1
      ? `${growth >= 0 ? "▲" : "▼"} ${PCT.format(Math.abs(growth))} na última competência`
      : "Sem comparação disponível";
}

function updateStoryKpis() {
  const revenue = sum(state.allRecords, "revenue");
  const entities = new Set(
    state.allRecords.map((r) => r.agency).filter(Boolean),
  ).size;
  const monthly = aggregate(state.allRecords, "month", "revenue").sort(
    (a, b) => monthKey(a.label) - monthKey(b.label),
  );
  const prev = monthly.at(-2)?.value || 0,
    last = monthly.at(-1)?.value || 0;
  $$('[data-kpi="records"]').forEach(
    (el) => (el.textContent = NUM.format(state.allRecords.length)),
  );
  $$('[data-kpi="revenue"]').forEach(
    (el) => (el.textContent = MONEY.format(revenue)),
  );
  $$('[data-kpi="entities"]').forEach(
    (el) => (el.textContent = NUM.format(entities)),
  );
  $$('[data-kpi="competences"]').forEach(
    (el) => (el.textContent = NUM.format(monthly.length)),
  );
}
...
function aggregate(rows, key, valueKey) {
  ...
  rows.forEach((r) =>
    map.set(
      r[key] || "Não informado",
      (map.get(r[key] || "Não informado") || 0) + (Number(r[valueKey]) || 0),
    ),
  );
...
function countBy(rows, key) {
  ...
  rows.forEach((r) =>
    map.set(
      r[key] || "Não informado",
      (map.get(r[key] || "Não informado") || 0) + 1,
    ),
  );
  return [...map].map(([label, value]) => ({ label, value }));
}
...
function renderCharts() {
  const monthlyData = aggregate(state.filtered, "month", "revenue").sort(
    (a, b) => monthKey(a.label) - monthKey(b.label),
  );
  renderMonthlyChart(
    $("#monthlyChart"),
  ...
  renderAgencyChart(
    $("#agencyChart"),
    aggregate(state.filtered, "agency", "revenue").sort(
      (a, b) => b.value - a.value,
    ),
  );
  renderFundChart(
    $("#statusChart"),
    aggregate(state.filtered, "fund", "revenue").sort(
      (a, b) => b.value - a.value,
    ),
  );
}
...
function emptyChart(el) {
  el.innerHTML =
    '<div class="chart-empty">Nenhum dado encontrado para os filtros selecionados.</div>';
}
function renderMonthlyChart(el, data, activeMonth = "") {
  ...
  const w = 900,
    h = 460,
    p = { l: 76, r: 22, t: 28, b: 78 };
  const max = Math.max(...data.map((d) => d.value), 1);
  const slot = (w - p.l - p.r) / Math.max(data.length, 1);
  const bw = Math.max(40, slot * 0.55);
  const y = (v) => h - p.b - (v / max) * (h - p.t - p.b);
  const grid = [0, 0.25, 0.5, 0.75, 1]
    .map((t) => {
      const yy = y(max * t);
      return `<line class="grid-line" x1="${p.l}" x2="${w - p.r}" y1="${yy}" y2="${yy}"/><text class="axis-label" x="${p.l - 12}" y="${yy + 4}" text-anchor="end">${compact(max * t)}</text>`;
    })
    .join("");
  const bars = data
    .map((d, i) => {
      const bh = (d.value / max) * (h - p.t - p.b);
      const xx = p.l + i * slot + (slot - bw) / 2,
        yy = h - p.b - bh;
...
function renderColumnChart(el, data, filterable = false) {
  ...
  const w = 900,
    h = 520,
    p = { l: 84, r: 28, t: 30, b: 58 };
  const max = Math.max(...data.map((d) => d.value), 1);
  const slot = (w - p.l - p.r) / Math.max(data.length, 1);
  const bw = Math.max(34, slot * 0.58);
  const y = (v) => h - p.b - (v / max) * (h - p.t - p.b);
  const grid = [0, 0.25, 0.5, 0.75, 1]
    .map((t) => {
      const yy = y(max * t);
  ...
  const bars = data
    .map((d, i) => {
      ...
      const xx = p.l + i * slot + (slot - bw) / 2,
        yy = h - p.b - bh;
      return `<rect class="bar monthly-bar" x="${xx}" y="${yy}" width="${bw}" height="${bh}" data-month="${escapeHtml(d.label)}" data-tip="${escapeHtml(d.label)} · ${escapeHtml(MONEY.format(d.value))}"/><text class="axis-label" x="${xx + bw / 2}" y="${h - 20}" text-anchor="middle">${escapeHtml(d.label)}</text>`;
    })
    .join("");
  el.innerHTML = `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none">${grid}${bars}</svg>`;
  bindTips(el);
  if (filterable) bindMonthFilter(el);
}
...
function bindTips(root) {
  $$("[data-tip]", root).forEach((node) => {
    ...
    node.addEventListener("focus", (e) => showTip(e, node.dataset.tip));
...
function showTip(e, text) {
  ...
  tip.style.left = Math.min(x + 14, innerWidth - tip.offsetWidth - 12) + "px";
  tip.style.top = Math.min(y + 14, innerHeight - tip.offsetHeight - 12) + "px";
}
function hideTip() {
...
function renderTable() {
  $("#resultCount").textContent =
    `${NUM.format(state.filtered.length)} registro(s) encontrado(s)`;
  $("#dataTable").innerHTML =
    state.filtered
      .slice(0, 250)
      .map(
    ...
    <td>${MONEY.format(Number(r.revenue) || 0)}</td></tr>`,
      )
...
function statusClass(status = "") {
  ...
  return s.includes("atras")
    ? "overdue"
    : s.includes("acomp") || s.includes("pend")
      ? "pending"
      : "";
}

function initStory() {
  const story = $("#narrativa"),
    panels = $$(".story-panel"),
  ...
  function update() {
    ...
    const p = Math.min(1, Math.max(0, -rect.top / Math.max(distance, 1)));
    progress.style.height = p * 100 + "%";
    const exact = p * (panels.length - 1),
      current = Math.min(panels.length - 1, Math.floor(exact + 0.5));
    state.scene = current;
    panels.forEach((panel, i) => {
      const delta = Math.abs(exact - i);
      const opacity = Math.max(0, 1 - delta * 1.35);
      panel.style.opacity = opacity;
      panel.style.transform =
        innerWidth <= 720
          ? `translateY(${(i - exact) * 38}px)`
          : `translateY(calc(-50% + ${(i - exact) * 52}px))`;
    ...
    const seal = $(".state-seal"),
      ...
      ring = $("#debtRing"),
      orbits = $$(".data-orbit");
    seal.style.opacity = String(
      0.46 + Math.max(0, 1 - Math.abs(exact - 0) * 0.55) * 0.38,
    );
    seal.style.transform = `translate(-50%,-50%) scale(${0.7 + Math.min(exact, 1) * 0.18}) rotate(${exact * 2}deg)`;
    orbits.forEach(
      (o, i) =>
        (o.style.transform = `rotate(${(i ? 38 : -12) + exact * (i ? 17 : -11)}deg) scale(${0.82 + Math.min(exact, 1) * 0.22})`),
    );
    setVisual(chart, exact, 3, 0.9, "translateY");
    setVisual(map, exact, 4, 0.9, "scale");
    setVisual(ring, exact, 5, 0.9, "scale");
    $("#topbar").classList.toggle(
      "is-solid",
      p > 0.97 || scrollY > innerHeight * 0.25,
    );
  }
  function setVisual(node, exact, target, spread, type) {
    const o = Math.max(0, 1 - Math.abs(exact - target) / spread);
    node.style.opacity = o;
    node.style.transform =
      type === "scale"
        ? `scale(${0.72 + o * 0.28})`
        : `translateY(${(1 - o) * 55}px)`;
  }
  addEventListener(
    "scroll",
    () => {
...
function initEvents() {
  ...
  $("#toggleColumns").addEventListener("click", (e) => {
    const on = $(".table-wrap").classList.toggle("show-optional");
    e.currentTarget.textContent = on ? "Ocultar processo" : "Exibir processo";
  });
  $("#clearMonthFocus")?.addEventListener("click", () => {
    $("#monthFilter").value = "";
    applyFilters();
  });
  const dialog = $("#adminDialog");
  $$("[data-open-admin]").forEach((btn) =>
